  ...
  return (
    <>
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.05 }}>
        ...
        <p className="mt-2 text-sm text-gray-500">Professional internships and industry exposure</p>
      </motion.div>
      <motion.div
      ...
      >
        {experiences.map((exp, idx) => (
          <motion.div
            ...
            className="group rounded-xl border border-white/5 bg-card p-6 transition-all hover:border-primary/20 hover:shadow-lg hover:shadow-primary/5 hover:-translate-y-0.5"
          >
            <div className="flex flex-col gap-5 sm:flex-row">
              ...
              <div className="flex-1 space-y-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <span className="flex items-center gap-1 rounded-md bg-primary/10 px-3 py-1 text-xs font-medium text-primary-light">
                ...
                <ul className="space-y-2">
                  {exp.points.map((point, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm leading-relaxed text-gray-400">
                      <span className="mt-1.5 block h-1.5 w-1.5 shrink-0 rounded-full bg-primary-light" />
                      {point}
                    </li>
                  ))}
                </ul>
              ...
              <button
                ...
                className="group/cert relative flex shrink-0 flex-col items-center overflow-hidden rounded-lg border border-white/5 bg-surface transition-all hover:border-primary/30 hover:shadow-lg hover:shadow-primary/5"
              >
                <div className="flex h-24 w-36 items-center justify-center p-2 sm:h-28 sm:w-44">
                  <img
                    src={exp.certificate}
                    alt="Certificate"
                    className="h-full w-full object-contain opacity-70 transition-all group-hover/cert:opacity-100 group-hover/cert:scale-105"
                    onError={(e) => {
                      (e.target as HTMLImageElement).style.display = 'none';
                    }}
                  />
                </div>
                <div className="flex w-full items-center justify-center gap-1 border-t border-white/5 py-1.5 text-xs font-medium text-primary-light">
