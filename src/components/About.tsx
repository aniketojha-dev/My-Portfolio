...
export default function About() {
  return (
    <div className="flex h-full items-center justify-center px-6 py-6 md:px-8 md:py-8">
      <div className="flex w-full max-w-5xl flex-col items-center gap-6 lg:flex-row lg:items-start lg:gap-10">
        <div className="flex-1">
          ...
          >
            <span className="inline-block rounded-full bg-gradient-to-r from-primary/20 to-secondary/20 px-4 py-1.5 text-xs font-medium text-primary-light">
          ...
          >
            <p className="text-base font-semibold text-gray-200">Software Developer</p>
            <p className="text-base font-semibold text-gray-200">AI/ML Engineer</p>
          </motion.div>
          ...
          <motion.div
            ...
            className="mt-4 rounded-xl border border-white/5 bg-card p-4"
          >
            <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-gray-500">Interests</h3>
            <div className="flex flex-wrap gap-1.5">
              {['Software Development', 'Artificial Intelligence', 'Machine Learning', 'Cybersecurity', 'System Design'].map((item) => (
                <span
                  key={item}
                  className="rounded-full border border-white/5 bg-surface px-3 py-1 text-xs text-gray-300 transition-colors hover:border-primary/20 hover:text-primary-light"
                >
                  {item}
                </span>
              ))}
          ...
          <motion.div
            ...
            className="mt-3 rounded-xl border border-white/5 bg-card p-4"
          >
            <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-gray-500">Beyond Coding</h3>
            <div className="flex flex-wrap gap-3">
              {hobbies.map((hobby) => (
                <span key={hobby.name} className="flex items-center gap-1.5 text-xs text-gray-400">
          ...
          >
            <a
              href="/resume.pdf"
              download
              className="inline-block rounded-lg border border-white/10 bg-white/5 px-4 py-2 text-xs font-medium text-gray-300 transition-colors hover:border-white/20 hover:text-white"
            >
              Download Resume
            </a>
          </motion.div>
        ...
        >
          <div className="relative">
            <div className="absolute -inset-4 rounded-2xl bg-gradient-to-br from-primary/20 via-secondary/10 to-transparent opacity-60 blur-2xl" />
            <div className="relative w-72 rounded-2xl border border-white/10 bg-card p-5 shadow-xl shadow-black/20">
                ...
                <div className="mt-4 w-full border-t border-white/5 pt-4">
                  ...
                  <div className="flex flex-wrap justify-center gap-1.5">
                    {techBadges.map((badge) => (
                      <span
                        key={badge.name}
                        className={`rounded-full bg-gradient-to-r ${badge.color} ${badge.text} border border-white/5 px-2.5 py-0.5 text-[11px] font-medium`}
                      >
                        {badge.name}
                      </span>
                    ))}
